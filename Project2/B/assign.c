...
int ASSIGN_lloyd(double** vectors, int num_of_vectors, double (*metric)(double*,double*,int), int* cluster_center_of_vector, double* distance_from_cluster_center, int* cluster_centers, int K, int d,list** clusters){
...
int ASSIGN_reverse_R_lsh(double** vectors, int num_of_vectors, double (*metric)(double*,double*,int),void*** lsh_hashtables, int* cluster_center_of_vector, double* distance_from_cluster_center, int* cluster_centers, int** buckets_of_current_centers,double*** g,int* r_values, int L, int K,int d,double lsh_w, int M,int LSH_k,int buckets,list** clusters){
	...
	//printf("max_center_distance");
	//scanf("%lf",&max_center_distance);
	...
	for(i=0;i<K;i++){
		for(j=0;j<L;j++){
			...
			buckets_of_current_centers[i][j]=HASH_g_i(g[j],vectors[cluster_centers[i]-1],r_values,lsh_w,d,M,LSH_k,buckets);
...
int ASSIGN_reverse_R_hypercube(double** vectors, int num_of_vectors, double (*metric)(double*,double*,int),void** hypercube, int** cube_hashtables, int* cluster_center_of_vector, double* distance_from_cluster_center, int* cluster_centers, int* vertices_of_current_centers,int HYPERCUBE_probes,int** probes_array,double** h,int* r_values,int K,int d,double cube_w,int M, int HYPERCUBE_k,int buckets,list** clusters){
	...
	for(i=0;i<K;i++){
		...
		for(j=0;j<HYPERCUBE_k;j++){
			...
			vertices_of_current_centers[i]+=vertices_of_current_centers[i]+cube_hashtables[0][bucket_number];
