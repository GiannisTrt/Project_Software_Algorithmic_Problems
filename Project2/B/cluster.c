...
int main(int argc, char* argv[]){
	...
	// Synepws exoyme k grammes toy pinaka afoy exoyme k synarthseis katakermatismoy h_i kai 
	// ka8e synarthsh hi ekfrazetai san dianysma me d to plh8os syntetagmenes.
	// Oi syntetagmenes aytes epilegontai tyxaia sto diasthma (0, w] opoy to w 8ewreitai gnwsto.  
	...
	// Pinakas me ta onomata twn dianysmatwn
	char** vector_names;
	
	// Diavasma kai elegxos orismatwn
	if(CLUSTER_ARGS_get_cluster_args(argc,argv,&input_file,&configuration_file,&output_file,&assignment,&update,&complete,&silhouette)<0)
		exit(-1);
	
	// To LSH frechet assignment den exei ylopoih8ei akoma
	if(assignment==LSH_FRECHET_ASSIGNMENT){
		
		printf("LSH frechet assignment not supported yet!\n");
		exit(0);
		
	}
	
	// To Mean Frechet update den exei ylopoih8ei akoma
	if(update==MEAN_FRECHET){
		
		printf("Mean Frechet update not supported yet!\n");
		exit(0);
		
	}

	// Uncoment gia emfanish diabasmatos orismatwn
	// CLUSTER_ARGS_print_cluster_args(input_file,configuration_file,output_file,assignment,update,complete,silhouette);
	
	CLUSTER_ARGS_get_cluster_parameters(configuration_file,DEFAULT_K,DEFAULT_L,DEFAULT_LSH_k,DEFAULT_HYPERCUBE_M,DEFAULT_HYPERCUBE_k,DEFAULT_HYPERCUBE_probes,&K,&L,&LSH_k,&HYPERCUBE_M,&HYPERCUBE_k,&HYPERCUBE_probes,&lsh_divider,&lsh_m,&lsh_w,&cube_m,&cube_divider,&cube_w,&max_iterations);
	...
	if(assignment==LSH_ASSIGNMENT){
		...
		for(i=0;i<num_of_vectors;i++){
			for(j=0;j<L;j++){
				...
				bucket_number=HASH_g_i(g[j],vectors[i],r_values,lsh_w,d,lsh_m,LSH_k,buckets);
	...
	// Epishs prepei na eisax8oun ta dianysmata ston yperkybo.	
		...
		// Dhmioyrgia synarthsewn katakermatismoy
		h=HASH_create_hash_function(HYPERCUBE_k,d,cube_w);
		...
		for(i=0;i<num_of_vectors;i++){
			...
			for(j=0;j<HYPERCUBE_k;j++){
				...
				// bucket_number=HASH_h_i(g[0][j],vectors[i],1/cube_w,d)%buckets;
	...
	while(1){
		...
		// Periptwsh assign me reverse R me lsh
			...
			ASSIGN_reverse_R_lsh(vectors,num_of_vectors,METRIC_L2,lsh_hashtables,cluster_center_of_vector,distance_from_cluster_center, cluster_centers,buckets_of_current_centers,g,r_values,L,K,d,lsh_w,lsh_m,LSH_k,buckets,clusters);
		...
		// Periptwsh assign me reverse R me hypercube
			...
			ASSIGN_reverse_R_hypercube(vectors,num_of_vectors,METRIC_L2,hypercube,cube_hashtables,cluster_center_of_vector,distance_from_cluster_center, cluster_centers,vertices_of_current_centers,HYPERCUBE_probes,probes_array,h,r_values,K,d,cube_w,M,HYPERCUBE_k,buckets,clusters);
	...
	//************ Ypologismos kai aksiologhsh silhouete (an zhteitai) ************//
	...
	if(silhouette){
		...
		for(cluster_num=0;cluster_num<K;cluster_num++){
			...
			for(i=0;i<current_cluster_size;i++){
				...
				if(current_cluster_size-1>0)
					a_i=sum_distance/(current_cluster_size-1);
				else
					a_i=0;
				
				if(neihbour_cluster_size)
	...
	if(complete){
		...
		for(i=0;i<K;i++){
			...
			cendroid=cluster_centers[i]-1;
			
			fprintf(fp,"CLUSTER-%d { %s",i,vector_names[cendroid]);
			
			cluster_size = ((bucket*)clusters[i])->length;
			
			for(j=0;j<cluster_size;j++)
				if((((bucket*)clusters[i])->keys)[j]!=cendroid)
					fprintf(fp,",%s ",vector_names[(((bucket*)clusters[i])->keys)[j]-1]);			
			fprintf(fp,"}\n");
	...
	if(assignment==LSH_ASSIGNMENT){
		...
		// Apeley8erwsh synarthsewn katakermatismoys
		HASH_free_hash_function(h,HYPERCUBE_k);
		//HASH_free_hash_functions(g,1,HYPERCUBE_k);
