...
int main(int argc, char* argv[]){
	...
	for(i=0;i<num_of_vectors;i++){
		for(j=0;j<L;j++){
			...
			bucket_number=HASH_g_i(g[j],vectors[i],r_values,w,d,M,k,buckets);
	...
	for(q=0;q<num_of_queries;q++){
		...
		for(j=0;j<L;j++){
			...
			search_bucket_numbers[j]=HASH_g_i(g[j],queries[q],r_values,w,d,M,k,buckets);
